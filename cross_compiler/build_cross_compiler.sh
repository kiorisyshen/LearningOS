...
############################
# Additionally, choose to build gdb
############################
mkdir -p $(pwd)/tmp/src
pushd $(pwd)/tmp/src

if ! [ -f $GDBNAME.tar.gz ]; then
    printf "$SHELL_FORE_GREEN\nDownloading $GDBNAME $SHELL_NC\n"
    curl -O http://ftp.gnu.org/gnu/gdb/$GDBNAME.tar.gz
fi

printf "$SHELL_FORE_GREEN\nExtracting $GDBNAME $SHELL_NC\n"
tar xf $GDBNAME.tar.gz
mkdir -p gdb-build
cd gdb-build
../$GDBNAME/configure --target="$TARGET" --prefix="$PREFIX" --program-prefix=i386-elf-

printf "$SHELL_FORE_GREEN\nBuilding $GDBNAME $SHELL_NC\n"
make -j4
make install
